...
def validate_sql_server_integration():
    ...
    pytds_patterns = [
        (
            r"TABLE_TYPE IN \('BASE TABLE', 'VIEW'\)",
            "Main query includes both tables and views",
        ),
        (
            r"List all tables and views",
    ...
    pytds_valid = check_file_content(
        pytds_file, pytds_patterns, "SQL Server Operator (database_helpers_pytds.py)"
    )
    ...
    main_valid = check_file_content(
        main_file, main_helpers_patterns, "Main Database Helpers (database_helpers.py)"
    )
    ...
    method_patterns = [
        (
            r"def list_tables\(self.*\) -> list\[str\]",
            "list_tables returns list of strings",
        ),
        (
            r"def list_views_only\(self.*\) -> list\[str\]",
            "list_views_only returns list of strings",
        ),
        (
            r"def list_tables_only\(self.*\) -> list\[str\]",
            "list_tables_only returns list of strings",
        ),
        (
            r"def list_tables_with_types\(self.*\) -> list\[dict",
            "list_tables_with_types returns list of dicts",
        ),
        (
            r"def get_object_type\(self.*\) -> str \| None",
            "get_object_type returns optional string",
        ),
    ]
