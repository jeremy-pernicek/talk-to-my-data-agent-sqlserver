...
def test_pushdown_config():
    ...
    try:
        ...
        test_queries = [
            ...
            ("SELECT col1, col2 FROM table WHERE id = 123", True, "Specific SELECT without aggregation"),
        ...
        custom_config = PushdownConfig(
            max_result_memory_mb=1000,
            auto_add_top_limit=10000,
...
def test_query_optimization():
    ...
    try:
        from database_helpers_pytds import SQLServerOperatorPytds, PushdownConfig
...
def test_streaming_methods():
    ...
    try:
        ...
        helper_methods = [
            '_might_return_large_results',
            '_execute_with_memory_check'
        ]
...
def test_prompt_enhancements():
    ...
    try:
        ...
        if len(found_keywords) >= len(pushdown_keywords) * 0.7:  # 70% threshold
            print(f"✓ Prompt contains {len(found_keywords)}/{len(pushdown_keywords)} pushdown optimization guidance")
        else:
            print(f"⚠ Prompt may need more pushdown guidance: {len(found_keywords)}/{len(pushdown_keywords)}")
...
def main():
    ...
    if passed == total:
        ...
        print("✅ Most tests passed. Minor issues may exist but functionality is largely working.")
    else:
